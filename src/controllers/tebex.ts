...
import {
  ...
  PremiumPlanPeriod,
  PremiumPlans,
} from "../entity/dash/PremiumPlans.js";
import {
  PremiumServices,
  PremiumServiceStatus,
  PremiumServiceType,
} from "../entity/dash/PremiumServices.js";
...
// adding a post
const handleTebexWebhook = async (req: Request, res: Response): Promise<void> => {
...
async function paymentManager(payment: PaymentWebhook | RecurringPaymentWebhook) {
  switch (payment.type) {
    case TebexWebhookType.SubscriptionCreate:
      ...
      await recieveSubscriptionPayment(payment);
      break;

    case TebexWebhookType.SubscriptionEnd:
      payment = payment as RecurringPaymentWebhook;

      await getConnection("dash").manager.update(
        PremiumServices,
    ...
    case TebexWebhookType.PaymentCreated:
      ...
      // eslint-disable-next-line no-case-declarations
      const existingPayment = await getConnection("dash").manager.findOne(Payments, {
        where: { payment_id: payment.subject.transaction_id },
      });
      ...
      for (const tebexPlan of payment.subject.products) {
        for (let i = 0; i < tebexPlan.quantity; i++) {
          // we have the get the plan id from the plan tebex id
          const currentPlan = await getConnection("dash").manager.findOne(PremiumPlans, {
            where: { tebexPackageId: String(tebexPlan.id) },
          });
          if (!currentPlan) {
            ...
            const matchingService = await getConnection("dash").manager.findOne(PremiumServices, {
              where: {
                user_id: payment.subject.customer.username.id,
                plan_id: currentPlan.id,
                subscriptionReference: undefined,
            ...
            if (matchingService) {
              ...
              if (currentPlan.period == PremiumPlanPeriod.MONTHLY) {
                matchingService.nextDue = dayjs(matchingService.nextDue)
                  .add(1, "month")
                  .format("YYYY-MM-DD");
              ...
              if (currentPlan.category == PremiumPlanCategory.GOLD) {
                ...
                sendMail(
                  payment.subject.customer.email,
                  "Your purchase on InviteLogger has been processed",
                  `Hello, your ${currentPlan.name} has been activated succesfully. To activate your private bot instance, join our support server and run the tutorial below.`,
                  {
                    title: "Thanks for your purchase",
                    button_txt: "Private bot setup tutorial",
                    button_url:
                      "https://docs.invitelogger.me/pbi/get-pbi/setup-your-private-bot-instance",
                  }
                );
              }
            }
    ...
    case TebexWebhookType.PaymentRefunded:
      ...
      await getConnection("dash").manager.update(
        ...
        {
          status: payment.subject.status.description,
          refundedAt: dayjs().format("YYYY-MM-DD  HH:mm:ss.000"),
        }
      );
...
async function recieveSubscriptionPayment(payment: RecurringPaymentWebhook) {
  ...
  // time to check if it is needed to extend a current
  for (const tebexPlan of payment.subject.last_payment.products) {
    for (let i = 0; i < tebexPlan.quantity; i++) {
      // we have the get the plan id from the plan tebex id
      const currentPlan = await getConnection("dash").manager.findOne(PremiumPlans, {
        where: { tebexPackageId: String(tebexPlan.id) },
      ...
      if (!currentPlan) {
        ...
        if (matchingService) {
          ...
          // create a new service
          const nextDue = dayjs(payment.subject.next_payment_at).format("YYYY-MM-DD");
